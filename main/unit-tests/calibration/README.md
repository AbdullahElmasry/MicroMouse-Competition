...
Open `encoder_ticks/encoder_ticks.ino`. Mark a straight 1260 mm track (7 x 180 mm), measured using the same reference point on the chassis at both ends.

1. Position the robot at the start, wheels on the normal maze surface.
2. Send `r` to zero both counts and begin recording.
3. Push slowly and straight forward for exactly 1260 mm. Avoid wheel slip, lifting, and reversing.
...
An MPU-6050 has no raw yaw-angle register. This sketch calibrates the stationary Z-axis bias at startup and prints the bias-corrected rate, the rate applied after the 0.5-degree/second deadband, and unwrapped relative yaw calculated as `yaw += rate * elapsed_seconds`. It applies no software smoothing, sensor fusion, or DMP. `DLPF_CFG=6` selects the MPU6050's 5 Hz hardware gyro filter. Register settings follow the manufacturer's [MPU-6000/MPU-6050 register map](https://www.invensense.com/wp-content/uploads/2015/02/MPU-6000-Register-Map1.pdf).
...
The reported angle is relative, not compass heading, and Z integration is only appropriate for level rotation. I2C failures invalidate the run. The serial output is reduced to 20 Hz for readability while valid samples are integrated every loop without software averaging. The deadband prevents small stationary noise from accumulating after movement.
