...
void setup() {
  ...
  Serial.println("c: clockwise 360, a: anticlockwise 360.");
...
void loop() {
  ...
  char command = Serial.read();
  if (command == 'c') runMove(4 * QUARTER_TURN_TICKS, -1, 1, false);
  if (command == 'a') runMove(4 * QUARTER_TURN_TICKS, 1, -1, false);
}
