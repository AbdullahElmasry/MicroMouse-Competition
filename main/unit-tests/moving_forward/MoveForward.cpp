...
class MpuYaw {
 public:
  ...
  void service() {
    ...
    // Match rotation.cpp: physical right rotation is positive.
...
#error Bluetooth Classic Serial is not available on this ESP32 target.
#endif

const char* BLUETOOTH_DEVICE_NAME = "MicroMouse";
...
constexpr float NO_WALL_MPU_HEADING_KP = 2.0f;
constexpr float NO_WALL_MPU_RATE_KD = 1.5f;
constexpr float NO_WALL_MPU_MAX_PWM = 15.0f;
...
MovementCommands bluetoothCommands;
...
void initBluetooth() {
    ...
    Serial.println();
    if (bluetoothReady) {
        Serial.print("Bluetooth Serial ready. Device name: ");
...
void handleBluetoothClient() {
    if (!bluetoothReady) {
        bluetoothCommands.reset();
        return;
    }

    const bool connected = SerialBT.hasClient();
    if (connected == bluetoothClientConnected) {
        return;
    }

    bluetoothClientConnected = connected;
    bluetoothCommands.reset();
    if (connected) {
        Serial.println();
        Serial.println("Bluetooth serial client connected.");
        SerialBT.println("=================================");
        SerialBT.println("ESP32 Micromouse connected by Bluetooth");
        SerialBT.println("Send s for 7 continuous cells; d stops the run. Front brake: 40 mm.");
        SerialBT.println("=================================");
    } else {
        Serial.println("Bluetooth serial client disconnected.");
    }
}
...
MovementCommand readCommand() {
    ...
    if (bluetoothReady && SerialBT.hasClient()) {
        for (int i = 0; i < 64 && SerialBT.available(); ++i) {
            result = mergeCommands(result, bluetoothCommands.feed(SerialBT.read(), millis()));
        }
        result = mergeCommands(result, bluetoothCommands.poll(millis()));
    }
...
void discardPendingCommands() {
    ...
    int bluetoothPending = bluetoothReady ? SerialBT.available() : 0;
    while (bluetoothPending-- > 0) SerialBT.read();
    usbCommands.reset();
    bluetoothCommands.reset();
}
...
void reportMovementConfig() {
    debugPrintln("Forward build: physical-motor-map-v7-bluetooth | compiled " __DATE__ " " __TIME__);
    ...
    debugPrintln(config);
    debugPrintln("MPU trim | one-wall Kp/Kd/max: 2.00/0.15/15 | no-wall Kp/Kd/max/weight: 2.00/1.50/15/80%");
}
...
bool runForwardDistance() {
    ...
    while (true) {
        ...
        if (millis() - lastLog >= 200) {
            // One complete line reduces the number of Bluetooth write calls.
            char telemetry[560];
            snprintf(telemetry, sizeof(telemetry),
                "ticks L/R: %lu/%lu | ToF mm F/Lfiltered/Rraw/Rfiltered-corrected: %d/%d/%d/%d | command L/R: %d/%d | remaining ticks: %lu | distance PID PWM: %d | case: %s | MPU yaw: %.2f deg | wall error: %.1f | wall PID PWM: %.2f | encoder error/PWM: %.1f/%.2f | MPU trim: %.2f | Ffiltered/Lraw: %d/%d | dt ms: %.1f | side walls detected: %d",
                left, right, front, sideLeft, sideRightRaw, sideRight,
                commands.left, commands.right, remaining, approachSpeed,
                mode==WallMode::Two?"two":(mode==WallMode::LeftOnly?"left+MPU":(mode==WallMode::RightOnly?"right+MPU":"none:encoder+MPU")),
    ...
    debugPrint(" | final MPU yaw: "); debugPrint(mpuYaw.yaw() * MPU_YAW_SIGN, 2);
    debugPrint(" deg");
...
void moveForwardSetup() {
    ...
    // Establish stopped motor outputs before enabling Bluetooth commands.
    initBluetooth();
