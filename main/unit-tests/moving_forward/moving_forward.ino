...
constexpr int CELL_LENGTH_MM = 180, FORWARD_CELLS = 8;
constexpr long FORWARD_TARGET_TICKS = CELL_TICKS * FORWARD_CELLS;
constexpr int FORWARD_TARGET_MM = CELL_LENGTH_MM * FORWARD_CELLS;
constexpr int FORWARD_SPEED = 70, TURN_SPEED = 90, TURN_SLOW_SPEED = 60;
constexpr int FRONT_STOP_MM = 100;
constexpr unsigned long MOVE_TIMEOUT_MS = 15000UL * FORWARD_CELLS;
...
void setup() {
  ...
  Serial.print("f: forward continuously for "); Serial.print(FORWARD_CELLS);
  Serial.print(" cells, target mm: "); Serial.print(FORWARD_TARGET_MM);
  Serial.print(", ticks per wheel: "); Serial.println(FORWARD_TARGET_TICKS);
...
void loop() {
  ...
  char command = Serial.read();
  if (command == 'f') runMove(FORWARD_TARGET_TICKS, 1, 1, true);
}
